...
def client(tmp_path):
    ...
    # Import dopo aver impostato SMF_BASE_PATH per evitare adapter globali puntati altrove.
    api_production = importlib.import_module("app.api_production")
    from app.smf.smf_adapter import SMFAdapter

    api_production.smf_adapter = SMFAdapter(base_path=tmp_path)
